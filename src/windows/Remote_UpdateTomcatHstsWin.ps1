        ...
        $invokeParams = @{
            ...
            ScriptBlock  = {
                ...
                function Test-FilterCompliant {
                    ...
                    # Try to find init-param elements
                    $initParams = $null
                    ...
                    if ($initParams) {
                        foreach ($param in $initParams) {
                            ...
                            foreach ($xpath in $nameXpaths) {
                                try { $name = $param.SelectSingleNode($xpath); if ($name) { break } } catch { }
                            }
                            foreach ($xpath in $valueXpaths) {
                                try { $value = $param.SelectSingleNode($xpath); if ($value) { break } } catch { }
                            }
                ...
                if ($failedPaths.Count -gt 0 -and $Mode -eq "audit") {
                    ...
                    foreach ($failedPath in $failedPaths) {
                        ...
                        Write-LogMessage "      .\Remote_UpdateTomcatHstsWin.ps1 -ServerName $env:COMPUTERNAME -Mode configure -SecurityLevel 1 -CustomPaths @('$failedPath')"
                        Write-LogMessage ""
                        Write-LogMessage "  [2] HIGH - OWASP Recommended (max-age=31536000; includeSubDomains):"
                        Write-LogMessage "      .\Remote_UpdateTomcatHstsWin.ps1 -ServerName $env:COMPUTERNAME -Mode configure -SecurityLevel 2 -CustomPaths @('$failedPath')"
                        Write-LogMessage ""
                        Write-LogMessage "  [3] VERY HIGH - Preload Ready (max-age=31536000; includeSubDomains; preload):"
                        Write-LogMessage "      .\Remote_UpdateTomcatHstsWin.ps1 -ServerName $env:COMPUTERNAME -Mode configure -SecurityLevel 3 -CustomPaths @('$failedPath')"
                        Write-LogMessage ""
                        Write-LogMessage "  [4] MAXIMUM - Highest Security (max-age=63072000; includeSubDomains; preload):"
                        Write-LogMessage "      .\Remote_UpdateTomcatHstsWin.ps1 -ServerName $env:COMPUTERNAME -Mode configure -SecurityLevel 4 -CustomPaths @('$failedPath')"
                        Write-LogMessage ""
                        $pathNum++
                    }
                    ...
                    Write-LogMessage ""
                    Write-LogMessage "  [1] Apply BASIC to ALL:      .\Remote_UpdateTomcatHstsWin.ps1 -ServerName $env:COMPUTERNAME -Mode configure -SecurityLevel 1"
                    
                    Write-LogMessage "  [2] Apply HIGH to ALL:       .\Remote_UpdateTomcatHstsWin.ps1 -ServerName $env:COMPUTERNAME -Mode configure -SecurityLevel 2"
                    Write-LogMessage "  [3] Apply VERY HIGH to ALL:  .\Remote_UpdateTomcatHstsWin.ps1 -ServerName $env:COMPUTERNAME -Mode configure -SecurityLevel 3"
                    Write-LogMessage "  [4] Apply MAXIMUM to ALL:    .\Remote_UpdateTomcatHstsWin.ps1 -ServerName $env:COMPUTERNAME -Mode configure -SecurityLevel 4"
                    Write-LogMessage ""
