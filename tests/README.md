...
- HSTS patching scripts in `src/windows/Patch/powershell/`
...
- Sudo/root access
- HSTS patching script in `src/unix/Patch/bash/`
...
- Verify HSTS patching scripts exist in `src/windows/Patch/powershell/`
- Check log file for detailed error messages: `$env:TEMP\TestHstsWin.log`

### Unix Tests Fail

- Ensure you have sudo/root access
- Verify HSTS patching script exists and is executable: `src/unix/Patch/bash/UpdateTomcatHstsUnix.sh`
...
**Windows (PowerShell):**
```powershell
$testResult = & .\tests\Patch\windows\test_hsts_win.ps1
if ($LASTEXITCODE -ne 0) { exit 1 }
```

**Unix (Bash):**
```bash
sudo ./tests/Patch/unix/test_hsts_unix.sh
